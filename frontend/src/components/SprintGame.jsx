...
export default function SprintGame({ topic, onExit, onPlayAgain }) {
  ...
  useEffect(() => {
    const savedBest = localStorage.getItem(`sprintBestScore_${topic}`);
  ...
  useEffect(() => {
    if ((isGameOver || isVictory) && score > bestScore) {
      setBestScore(score);
      localStorage.setItem(`sprintBestScore_${topic}`, score);
    }
  }, [isGameOver, isVictory, score, bestScore, topic]);
  ...
  // Auto-Advance / Run-End Countdown Trigger (3 seconds delay after answering)
  useEffect(() => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
          ...
          <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-right justify-end w-full sm:w-auto">
            <div className="flex gap-4">
              ...
              <div>
                <div className="text-[10px] text-pink-400 font-bold uppercase tracking-widest">Score</div>
        ...
        {/* --- DYNAMIC VIEW: IF GAME OVER OR VICTORY --- */}
        {isGameOver || isVictory ? (
          ...
          >
            ...
            <h2 className="text-4xl font-black text-white mb-2 uppercase tracking-tight">
              {isVictory ? "Sprint Cleared" : "Run Terminated"}
            </h2>
            <p className="text-zinc-400 mb-8 font-medium">
              {isVictory ? `You survived the ${topic} arena.` : `You ran out of lives in ${topic}.`}
            </p>
          ...
          <>
            ...
            {/* Tactical Pass Button OR Dynamic Countdown Feedback */}
